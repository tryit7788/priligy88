  ...
  useEffect(() => {
    const handleFilterChange = async (event: CustomEvent) => {
      ...
      try {
        ...
        const response = await fetch(`/api/products.json?${queryParams.toString()}`);
    ...
    window.addEventListener('filterchange', handleFilterChange as unknown as EventListener);
    return () => {
      window.removeEventListener('filterchange', handleFilterChange as unknown as EventListener);
    };
